...
C ##################################################################################################
      SUBROUTINE TREE_COMPP_PB(p,kappa,eps,tpoten,poten)
      use treecode
      IMPLICIT NONE
C
...
C      REAL(KIND=r8),DIMENSION(numpars),INTENT(INOUT) :: tpoten
      REAL*8 :: kappa,eps
      REAL*8 :: tpoten(2*numpars), poten(2*numpars)
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		CALL COMPP_TREE(p,peng,x,y,z,schg,tpoten,
     &		kappa,theta,numpars,kk,eps,tempq,der_cof)
       poten(i)=pre1*peng_old(1)-peng(1)
       poten(numpars+i)=pre2*peng_old(2)-peng(2)
...
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C      RECURSIVE SUBROUTINE NODEMATVEC(p, )
C        IMPLICIT NONE


C      END SUBROUTINE
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      RECURSIVE SUBROUTINE LEAFLENGTH(p,idx,ibeg,iend,nrow)
        USE treecode
        IMPLICIT NONE

        TYPE(tnode),POINTER :: p
        INTEGER :: idx, i, ibeg, iend, nrow

        if ( idx .EQ. p%ibeg .AND. p%num_children .EQ. 0 ) THEN
          ibeg = p%ibeg
          iend = p%iend
          nrow = p%numpar
        else
          if ( p%num_children .NE. 0 ) THEN
            do i=1,p%num_children
              CALL LEAFLENGTH(p%child(i)%p_to_tnode,idx,ibeg,iend,nrow)
            END DO
          endif
        endif
C        print *,ibeg, iend, nrow
      END SUBROUTINE
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE TREE_COMPP_PB_prec(p,kappa,eps,tpoten,poten)
        use treecode
        IMPLICIT NONE
C
C TREE_COMPF is the driver routine which calls COMPF_TREE for each
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          ...
          CALL COMPP_TREE_prec(p,peng,x,y,z,schg,tpoten,
     &		kappa,theta,numpars,kk,eps,tempq,der_cof,i)
          poten(i)=pre1*peng_old(1)-peng(1)
          poten(numpars+i)=pre2*peng_old(2)-peng(2)
C          print *,peng(1),peng(2)
C		       call cpu_time(time2)
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          x(i)=tempx
          tr_area(i)=temp_area
C          print *,poten(i),poten(numpars+i)
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
C		print *,'cpu time for one p-c intercation: ',i,real(time2-time1)
        ENDDO
C        STOP
        ...
        RETURN
      END SUBROUTINE TREE_COMPP_PB_prec
