...
!######################################################
   ABSTRACT INTERFACE
     FUNCTION matvec_template( N, XX, bb )
       integer, INTENT(in) :: N
       real*8, INTENT(in) :: xx(N),bb(N)
     END FUNCTION matvec_template

     FUNCTION msolve_template( N, XX, bb )
       integer, INTENT(in) :: N
       real*8, INTENT(in) :: xx(N),bb(N)
     END FUNCTION msolve_template
   END INTERFACE
...
MODULE COMDATAprec
   ...
   integer, dimension(:), allocatable :: indx
!######################################################
   integer,allocatable,dimension(:):: iworkp,IGWKp
   real*8,allocatable,dimension(:):: rworkp,sb,sx,RGWKp
!######################################################
   ABSTRACT INTERFACE
     FUNCTION matvec_template( N, XX, bb )
       integer, INTENT(in) :: N
